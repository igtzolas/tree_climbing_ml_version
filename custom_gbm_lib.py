...
class ResidualsCalculator(nn.Module):
    ...
    def loss_classifier(self, targets):
        ...
        probabilities = 1.0 / (1.0 + torch.exp(-logodds))
        loss = len(targets) * F.binary_cross_entropy(probabilities, targets)
        return loss
    def loss_regressor(self, targets):
        values = self.forward()
        # Better provide the squared error and not the mean one for numerical stability in large datasets
        loss = F.mse_loss(values, targets) * targets.shape[0]
        return loss  
...
def fit_regression_tree_classifier_to_residuals(X_data, y_data, max_depth): # y_data -> residuals
    ...
    if type(X_data) == pd.DataFrame:
        for i in range(X_data.shape[0]):
            leaf_buckets.append(tuple(tree_regressor.decision_path(X_data.iloc[i, :].values.reshape(1, -1)).todok().keys()))
    else:
        for i in range(X_data.shape[0]):
...
class PytorchBasedGenericGradientBoost():
    ...
    def minimize_loss_function(self, targets, previous_predictions):
        ...
        for training_epoch in range(self.minimizer_training_epochs):
            ...
            loss = self.minimizer.loss(previous_predictions, targets_leaf_tensor)
    ...
    def compute_residuals(self, targets, predicted_values):
        ...
        return residuals
    def fit(self, X, y):
        # copy works on both numpy arrays and pandas dataframes
        X_values = X.copy()
        y_values = y.copy()
        ...
        # yield {"probability" : torch.sigmoid(torch.tensor(prediction_values)), "logodds" : prediction_values }
        if self.type == "regressor":
            X.loc[:, "Linear_0"] = prediction_values
        else:
            X.loc[:, "Logodds_0"] = prediction_values
            X.loc[:, "Probs_0"] = 1 / (1 + np.exp(-prediction_values))

        for classifier_index in range(self.n_trees):
            # Step 1 is to take the gradients/residuals
            self.regression_trees.append({"tree_index": classifier_index})
            gradients = residuals = self.compute_residuals(FloatTensor(y_values), FloatTensor(prediction_values))
            
            # Step 2 is to create the Decision tree on the residual values (grouping of examples)
            ...
            prediction_values_temp = np.zeros(prediction_values.shape)
            predictions_per_leaf_temp = np.zeros(prediction_values.shape)
            cluster_indices = np.zeros(prediction_values.shape, dtype=np.dtype("int"))

            # Step 3 L Update the prediction for each cluster of leaves
            for cluster_id, unique_cluster in enumerate(unique_clusters):
                ...
                prediction_values_temp[np.array(indices) == 1] = predictions_for_leaf_array
                predictions_per_leaf_temp[np.array(indices) == 1] = predictions_per_leaf_temp[np.array(indices) == 1] + prediction_for_leaf
                cluster_indices[np.array(indices) == 1] = cluster_id
                
            prediction_values = prediction_values_temp
            X.loc[:, f"Gradients_{classifier_index}_{classifier_index+1}"] = gradients.numpy()
            X.loc[:, f"Clusters_{classifier_index+1}"] = cluster_indices
            if self.type == "regressor":
                X.loc[:, f"Linear_{classifier_index + 1}"] = prediction_values
                X.loc[:, f"Leaf_Prediction_{classifier_index + 1}"] = predictions_per_leaf_temp
            else:
                X.loc[:, f"Logodds_{classifier_index + 1}"] = prediction_values
                X.loc[:, f"Probs_{classifier_index + 1}"] = 1 / (1 + np.exp(-prediction_values))
                X.loc[:, f"Leaf_Prediction_{classifier_index + 1}"] = predictions_per_leaf_temp
            
        return X, y
            # yield {
            #     "probability" : torch.sigmoid(torch.tensor(prediction_values)), 
            #     "logodds" : prediction_values,
            #     "residuals" : residuals 
            #     }    
    def predict(self, X):
        predictions = []
